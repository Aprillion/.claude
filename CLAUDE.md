...
- top level agent should remain unblocked to talk to me (and orchestrate subagents who will investigate and run shells and edit files)
  ...
  - after starting subagents or background shell, arm a monitor that will ping you every 10 minutes to tell you real time (not just elapsed wall clock) and latest output (not number of lines) making sure everyone is on task without side-quest joyride / doom loops
    - arm `python3 ~/notes/monitor/tick.py <session>/tasks/ <cli-output-files>` in the Monitor tool: it prints facts only (clock and gap since previous tick, seconds since each transcript's last event, the tool call still waiting for its result, last 3 calls, last sentence said) and the judgement stays with you; one monitor per session, TaskStop it when its reason is gone (`~/notes/monitor/README.md`)
...
- for reviewers: a paragraph of comment about a workaround usually indicates the code is wrong - same for timing hacks (setTimeout/rAF/settle windows)
  ...
  - independent models are available via Cursor CLI: `agent -p --model cursor-grok-4.6-medium` is basically for free (think of it as Sonnet-level subagent), `agent -p --model gpt-5.6-sol-high` for precise instruction following and completing hard work (Fable-level, but questionable taste in code style)
  - Jev (TypeSafe, `TYPESAFE_API_KEY`): a fast calibrated yes/no classifier, about 5 cents per 2000 transcript prompts; use it to screen large corpora for decisions, refusals or complaints and then read the hits; not for judgement about code (see `~/notes/jev.md`, tools in `~/notes/find-pluggable/tools/jev_*.py`)
  - before calling a report, commit message, or batch of code comments finished, run `python3 ~/notes/prose_review.py FILE` (`-` reads stdin, e.g. `git log -1 --format=%B | python3 ~/notes/prose_review.py -`; Kimi K2 via OpenRouter) and act on the findings - the author model cannot see their own slop habits
